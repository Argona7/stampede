...
run(
    f"ffmpeg -v error -y -ss {web_in} -t {web_dur:.2f} -i '{RAW}/web-walkthrough.webm' -i '{OUT}/cap-web.png' -filter_complex \"[1:v]format=rgba,fade=t=out:st=3.4:d=0.5:alpha=1[c];[0:v][c]overlay=0:main_h-overlay_h:enable='lte(t,3.9)',format=yuv420p\" -r 30 -c:v libx264 -crf 18 -pix_fmt yuv420p -movflags +faststart '{OUT}/stampede-v5-autopilot.mp4'"
)
...
for t in (2.0, 5.0, 10.0, 17.0, 24.0, 31.0, 36.5):
