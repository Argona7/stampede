...
- [x] `demo/v5/out/stampede-v5-autopilot.mp4` (16.7 MB) ships as a release asset of `v0.1.0`; `demo/v5/raw/web-walkthrough.webm` stays local. README links the asset URL.

## Configuration

- [x] `.env.example` documents `ALCHEMY_KEY` (ingest/live/verify), `HYPERSYNC_TOKEN` (optional), `PUBLIC_RPC`, `STAMPEDE_DB`, `TWITTERAPI_KEY` (optional), `STAMPEDE_DEMO_URL` (optional override of the release asset URL baked into `stampede/demo.py`).
...
- [x] The 17 MB bundle is a release asset of `v0.1.0`; `stampede demo` downloads it on first start (`DEFAULT_BUNDLE_URL` in `stampede/demo.py`, override with `--bundle-url` / `STAMPEDE_DEMO_URL`). `data/demo/*.xz` is ignored so it never enters the history.
...
- [x] Owner decision 2026-09-11 19:19: publish everything to the owner's GitHub → `Argona7/stampede`, public, full history (20 commits, ~120 MB packed, mostly demo frames and videos under `demo/`). Slimming the history was not requested and was not done.
